...
public class Map {
	...
	public String get(Point point) {
		...
		// Verifique se as coordenadas estão dentro do limite do mapa
		if (x < 0 || y < 0 || x >= scenario.length || y >= scenario[0].length) {
			return null; // Retorna null se estiver fora do limite
		}
		return scenario[x][y]; // Retorna o conteúdo da célula
	}


	public void moveRobot(Point nextPoint) {
		// Verifique se nextPoint está dentro dos limites do mapa
		if (nextPoint.getPositionX() < 0 || nextPoint.getPositionY() < 0 ||
				nextPoint.getPositionX() >= scenario.length || nextPoint.getPositionY() >= scenario[0].length) {
	...
	//metodo criado para buscar as posiçoes dos baus de tesouro
